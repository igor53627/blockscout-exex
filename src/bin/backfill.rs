//! Backfill tool - reads from JSON-RPC and indexes into the sidecar DB
//!
...
#[command(about = "Backfill index database from JSON-RPC")]
struct Args {
...
async fn main() -> Result<()> {
    ...
    let index_db = IndexDb::create(&args.index_path)?;

    let rpc = RpcClient::new(&args.rpc_url);

    // Determine block range
    let from_block = if args.from_block == 0 {
        index_db.last_indexed_block()?.map(|b| b + 1).unwrap_or(0)
    } else {
        args.from_block
    };
    ...
    for batch_start in (from_block..=to_block).step_by(args.batch_size as usize) {
        ...
        batch.set_last_indexed_block(batch_end)?;
